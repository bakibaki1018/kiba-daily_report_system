...
<c:import url="/WEB-INF/views/layout/app.jsp">
    <c:param name="content">
        ...
        <h2>日報 一覧</h2>
        <h3>
            <a href="<c:url value='/follows/index' />">フォロー中の日報のみ表示</a>
        </h3>
                        ...
                        <td class="report_follow"><c:if
                                ...
                                <c:choose>
                                    <c:when test="${followMap[report_id]}">
                                        <form method="POST"
                                            action="<c:url value="/follows2/destroy" />">
                                    ...
                                    <c:otherwise>
                                        <form method="POST"
                                            action="<c:url value="/follows2/create" />">
                                            <input type="hidden" name="_token" value="${_token}" /> <input
                                                type="hidden" name="follow_id" value="${report.id}" />
                                            <button class="followbutton" class="followbutton"
                                                type="submit">フォローする</button>
                                        </form>
                                    </c:otherwise>
                                </c:choose>
                            </c:if></td>
